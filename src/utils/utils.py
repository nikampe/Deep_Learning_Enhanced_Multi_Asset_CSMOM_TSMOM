...
def csmom_train_valid_test_split(dfs, df_returns, train_interval, train_valid_split, rebalance_freq, vol_lookback, vol_target):
    ...
    for i in range(0, len(split_indices)-2):
        ...
        y_arr_indices_train.append(y_indices_train)
        y_arr_indices_valid.append(y_indices_valid)
...
def tsmom_train_valid_test_split(lookback, dfs, df_returns, train_valid_split, rebalance_freq, vol_lookback, vol_target):
...
def plot_model_performances(scaled_cumulative_returns, labels, strategy, save=False):
    ...
    ax.set_xlabel("Time", size=12)
    ax.set_ylabel("Cumulative Return", size=12)
    ax.set_title("Model Backtesting - Cumulative Returns", size = 16)
    ax.spines['right'].set_color('none')
    ax.spines['top'].set_color('none')
...
def plot_model_performances_log_scale(scaled_cumulative_returns, labels, strategy, save=False):
    ...
    ax.set_xlabel("Time", size=12)
    ax.set_ylabel("Cumulative Return", size=12)
    ax.set_title("Model Backtesting - Cumulative Returns (Log-Scale)", size = 16)
    ax.set_yscale('log')
    ax.spines['right'].set_color('none')
    ax.spines['top'].set_color('none')
    ax.legend(loc = "upper left", frameon = False)
